...
import {
    User, Mail, Phone, MapPin, BookOpen, Users, Home, Building2,
    Calendar, Edit3, Check, X, ShieldCheck, BadgeInfo, KeyRound, Eye, EyeOff
} from 'lucide-react';
...
function InfoRow({ icon: Icon, label, value, accent = 'bg-indigo-50 text-indigo-600' }) {
    return (
...
const StudentInfo = () => {
    ...
    const fetchProfile = async () => {
        try {
            ...
            setForm({
                ...
                guardianContact: res.data.data.guardianContact || '',
            });
    ...
    return (
        <div className="space-y-6 max-w-2xl">
            ...
            {/* Profile banner */}
            <div className="bg-gradient-to-br from-blue-600 via-blue-500 to-indigo-600 rounded-2xl p-6 flex items-center gap-5 relative overflow-hidden">
                ...
                <button onClick={() => setEditing(!editing)} className="ml-auto bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
                    <Edit3 className="h-3.5 w-3.5" /> {editing ? 'Cancel Edit' : 'Edit Profile'}
                </button>
            </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                    ...
                    <form onSubmit={handleSave} className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div><label className={lc}>Phone Number</label><input value={form.phoneNumber} onChange={e => setForm({ ...form, phoneNumber: e.target.value })} className={ic} placeholder="10-digit number" /></div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className={lc}>Date of Birth</label><input type="date" value={form.dateOfBirth} onChange={e => setForm({ ...form, dateOfBirth: e.target.value })} className={ic} /></div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className={lc}>Guardian Name</label><input value={form.guardianName} onChange={e => setForm({ ...form, guardianName: e.target.value })} className={ic} placeholder="Parent/Guardian" /></div>
                            <div><label className={lc}>Guardian Contact</label><input value={form.guardianContact} onChange={e => setForm({ ...form, guardianContact: e.target.value })} className={ic} placeholder="Guardian phone" /></div>
                        </div>
                        <div className="flex gap-3 pt-1">
                            <button type="submit" disabled={saving} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-all active:scale-95 disabled:opacity-60">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-4 flex items-center gap-2">
                    <BadgeInfo className="h-3.5 w-3.5 text-indigo-400" /> Account Details
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <InfoRow icon={User} label="Full Name" value={profile?.name} />
                    <InfoRow icon={Mail} label="Email Address" value={profile?.email} accent="bg-slate-100 text-slate-600" />
                    <InfoRow icon={BookOpen} label="Student ID" value={profile?.studentId} accent="bg-indigo-50 text-indigo-600" />
                    <InfoRow icon={Phone} label="Phone Number" value={profile?.phoneNumber} accent="bg-emerald-50 text-emerald-600" />
                    <InfoRow icon={Calendar} label="Date of Birth" value={profile?.dateOfBirth ? new Date(profile.dateOfBirth).toLocaleDateString('en-IN') : null} accent="bg-violet-50 text-violet-600" />
                    <InfoRow icon={MapPin} label="Address" value={profile?.address} accent="bg-amber-50 text-amber-600" />
                    <InfoRow icon={User} label="Guardian Name" value={profile?.guardianName} accent="bg-rose-50 text-rose-500" />
                    <InfoRow icon={Phone} label="Guardian Contact" value={profile?.guardianContact} accent="bg-rose-50 text-rose-500" />
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-4 flex items-center gap-2">
                    <Home className="h-3.5 w-3.5 text-blue-400" /> Room Assignment
                </p>
                {profile?.roomAssigned ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <InfoRow icon={Home} label="Room No." value={profile.roomAssigned.roomNumber} accent="bg-blue-50 text-blue-600" />
                        <InfoRow icon={Building2} label="Block" value={profile.roomAssigned.block} accent="bg-blue-50 text-blue-600" />
                        <InfoRow icon={Building2} label="Floor" value={`Floor ${profile.roomAssigned.floor}`} accent="bg-blue-50 text-blue-600" />
                        <InfoRow icon={Users} label="Capacity" value={`${profile.roomAssigned.capacity} beds`} accent="bg-blue-50 text-blue-600" />
                    </div>
                ) : (
                    <div className="text-center py-6">
                        <Home className="h-8 w-8 text-slate-300 mx-auto mb-2" />
                        <p className="text-sm text-slate-400">No room assigned yet. Contact hostel admin.</p>
                    </div>
                )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <InfoRow icon={ShieldCheck} label="Role" value="Student" accent="bg-indigo-50 text-indigo-600" />
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide flex items-center gap-2">
                        <KeyRound className="h-3.5 w-3.5 text-indigo-400" /> Security
                    </p>
                    <button
                        onClick={() => { setShowPwSection(!showPwSection); setPwForm({ newPassword: '', confirmPassword: '' }); }}
                        className={`text-xs font-semibold px-3 py-1.5 rounded-xl transition-colors ${showPwSection ? 'bg-slate-100 text-slate-600' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'
                            }`}>
                ...
                {showPwSection && (
                    <form onSubmit={handleChangePassword} className="space-y-4 mt-4">
                        ...
                        <button type="submit" disabled={pwSaving || pwForm.newPassword !== pwForm.confirmPassword}
                            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-sm font-semibold rounded-xl transition-all active:scale-95">
